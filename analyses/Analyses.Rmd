...
plot.corpusSummary2 <- df.all_preferences %>% 
  filter(imputed_ditransitive_rate > 0) %>% 
...
fit.skew <- lm(imputed_DO_skew ~ log(imputed_ditransitive_rate), 
     data = (df.all_preferences %>% filter(imputed_ditransitive_rate >0)))
summary(fit.skew)
...
plot.corpusSummary3 <- df.all_preferences %>% 
  ...
  ggplot(aes(x = imputed_DO_skew, 
             y = redHerring_DO_skew)) + 
  geom_abline(slope = 1, 
              intercept = 0, 
              lty = 3) + 
  geom_point(aes(
    alpha = log(imputed_ditransitive_rate))) + 
  geom_text_repel(aes(label = verbLemma)) + 
  labs(
...
fit.random_herring <- lm(estimated_intercept ~ redHerring_DO_skew + imputed_DO_skew, 
                  data = df.bresnanBays_randomIntercepts)
